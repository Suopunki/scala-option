...
class TestSomeOptionTuple:
    ...
    def test_map(self):
        option = some((1, 2, 3))
        result = option.map(lambda x: tuple(i * 2 for i in x))
        assert result.get() == (2, 4, 6)

    def test_flat_map(self):
        option = some((1, 2, 3))
        result = option.flat_map(lambda x: some(tuple(i * 2 for i in x)))
        assert result.get() == (2, 4, 6)

    def test_fold(self):
        option = some((1, 2, 3))
        result = option.fold(lambda: (), lambda x: tuple(i * 2 for i in x))
        assert result == (2, 4, 6)
    ...
    def test_to_list(self):
        option = some((1, 2, 3))
        assert option.to_list() == [(1, 2, 3)]  # SomeOption of tuple becomes a list with tuple inside
